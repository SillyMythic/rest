...
settings := {
    ...
    hotbarRapidTriggerSlots: "2",
    hotbarShield: false,
    hotbarShieldSlots: "5",
    hotbarShieldDelay: 100,
    ; Rapid Trigger
    rapidTrigger: false,
    rapidTriggerKeybind: "LButton",
    rapidTriggerDelay: 50,
    rapidTriggerFirstPerson: true,
    rapidTriggerRequireRoblox: true,
    ; Custom
    customPrefix: "",
    customThemeName: "",
    customThemeDark: "",
...
Rest.group(restInterface, "Hotbar", 305, 90, 285, 445, "macro")
Rest.toggle(restInterface, 320, 130, "Enabled", settings.hotbar, "macro", (v) => setSetting("hotbar", v), "Tracks your hotbar slots to determine whether you're using a sniper or not.")
Rest.edit(restInterface, 320, 160, 255, "Sniper Slots", settings.hotbarSniperSlots, "macro", (v) => setSetting("hotbarSniperSlots", v), "Hotbar slots that are set to snipers.")
Rest.toggle(restInterface,320, 235, "Shield", settings.hotbarShield, "macro", (v) => setSetting("hotbarShield", v), "Immediately equips shield after sniping.")
Rest.edit(restInterface, 320, 265, 255, "Shield Slot", settings.hotbarShieldSlots, "macro", (v) => setSetting("hotbarShieldSlots", v), "Hotbar slot that is set to shield.")
Rest.slider(restInterface, 320, 325, 255, "Shield Time", 100, 1000, settings.hotbarShieldDelay, "macro", (v) => setSetting("hotbarShieldDelay", v), "Delay in ms to wait while the shield is equipped.")
Rest.edit(restInterface, 320, 400, 255, "Rapid Trigger Slots", settings.hotbarRapidTriggerSlots, "macro", (v) => setSetting("hotbarRapidTriggerSlots", v), "Hotbar slot that is set to rapid trigger.")
Rest.toggle(restInterface, 320, 490, "Require Roblox", settings.hotbarRequireRoblox, "macro", (v) => setSetting("hotbarRequireRoblox", v), "Ensures you're in first person before executing.")

Rest.group(restInterface, "Rapid Trigger", 10, 544, 285, 235, "macro")
Rest.toggle(restInterface, 25, 584, "Enabled", settings.rapidTrigger, "macro", (v) => setSetting("rapidTrigger", v), "Repeatedly shoots semi-automatic weapons.")
Rest.keybind(restInterface, 210, 584, settings.rapidTriggerKeybind, "macro", (v) => updateRapidTriggerBind(v), "Key used to trigger rapid trigger.")
Rest.slider(restInterface, 25, 614, 255, "Repeat Delay", 1, 100, settings.rapidTriggerDelay, "macro", (v) => setSetting("rapidTriggerDelay", v), "Delay in ms between shots.")
Rest.toggle(restInterface, 25, 699, "First Person", settings.rapidTriggerFirstPerson, "macro", (v) => setSetting("rapidTriggerFirstPerson", v), "Ensures you're in first person before executing.")
Rest.toggle(restInterface, 25, 729, "Require Roblox", settings.rapidTriggerRequireRoblox, "macro", (v) => setSetting("rapidTriggerRequireRoblox", v), "Ensures Roblox is the active window before executing.")
...
updateRapidTriggerBind(v) => updateBind("rapidTriggerKeybind", v, rapidTrigger, "~*", "", true)
...
updateRapidTriggerBind(settings.rapidTriggerKeybind)
...
isSemiEquipped(key) {
    for slot in StrSplit(settings.hotbarRapidTriggerSlots, ",")
        if (Trim(slot) = String(key))
            return true
    return false
}
...
sniperQuickscope(*) {
    ...
    if (settings.sniperBindMerge) {
        ...
        if (settings.sniperFirstPerson && (x != centerWidth || y != centerHeight)) {
            isMouse := true
        } else if (!settings.hotbar || isSniperEquipped(lastNumber)) {
            isSniper := true
        } else {
            isAR := true
        }
    ...
    if (isSniper) {
        ...
        if ((settings.hotbar && (A_TickCount - lastNumberTime < settings.sniperEquipDelay || !isSniperEquipped(lastNumber)))
...
rapidTrigger(*) {
    global isRapidTriggering, lastNumber, centerWidth, centerHeight


    if (!settings.rapidTrigger)
        return

    if (settings.rapidTriggerRequireRoblox && !WinActive("Roblox") || !ProcessExist("RobloxPlayerBeta.exe"))
        return

    if (settings.rapidTriggerFirstPerson) {
        MouseGetPos &x, &y
        if (x != centerWidth || y != centerHeight)
            return
    }
        
    if (settings.hotbar && !isSemiEquipped(lastNumber))
        return

    isRapidTriggering := true
    while GetKeyState(settings.rapidTriggerKeybind, "P") {
        SendInput "{LButton}"
        Sleep(settings.rapidTriggerDelay)
    }
    isRapidTriggering := false
}
